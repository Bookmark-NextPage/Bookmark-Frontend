...
export default function SearchBox() {
  ...
  const { data: records = [], isFetching } = useQuery({
    queryKey: ['recordSearch', debouncedQ],
    queryFn: () => searchRecords(debouncedQ),
  ...
  const showDropdown = focused && q.trim().length > 0;
  const loading = isFetching && records.length === 0;
          ...
          {loading ? (
            ...
            records.map((r) => (
              <div
                ...
                onMouseDown={(e) => {
                  e.preventDefault();
                  navigate(`/record/${r.recordId}`);
                  setQ('');
                }}
              >
                <div className="sh-title">
                  {r.title ? highlight(r.title, debouncedQ) : '제목 없는 기록'}
                </div>
                <div className="sh-sub">
                  # {r.chapterName} · {formatDate(r.createdAt)}
                </div>
              </div>
            ))
          )}
